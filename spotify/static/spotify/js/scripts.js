...
$(document).ready(function()
{
 ...
 $('#q').bind('typeahead:select', function(ev, suggestion) {
    ...
    success: function(data, jqXHR){
        ...
        $('#sug').append("<div><textarea rows='5' cols='50' id='msg' name='msg' height='300px' form='sugform'\
        ...
        <input type='hidden' name='spotifyuri' value='{{ suggestion.uri }}'>\
        <button class='btn btn-outline-secondary' type='submit' id='bton'\
         value='Submit' form='sugform'>Recommend!</button></form></div>");
  ...
  $('#displayrec').html('<div><iframe src=' + urlembed[0] + 
   ...
   $(function() {
    ...
    success: function(data, jqXHR){
        ...
        $("#genurl").append('<div class="input-group mb-3 shadow-textarea ">\
            <div align="middle" class="input-group-prepend shadow-textarea">\
            <textarea class="form-control" style="max-width:500px;" id="copy" rows="2" cols="40" > localhost:8000/listentothis/'+ data.genurl + '</textarea></div>\
    ...
    $(sugforminfo).removeClass('error');
    $(sugforminfo).addClass('success');
