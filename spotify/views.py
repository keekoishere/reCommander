...
def logout(request):
	...
	return render(request, 'spotify/index.html', {'alert':'Dont forget to log out of Spotify as well if you want to change accounts!'})


def needusername(request):
	return render(request, 'spotify/needusername.html')
...
def login(request):
	if request.method =='POST':
		...
		try:
			...
			if y.email != r['email']:
				return render(request, 'spotify/index.html', {'alert' : "That username already exists, and it isnt you!"})
			else:
				return HttpResponseRedirect('/login')
		except:
			pass
	...
	try:
		...
		try:
				if r['display_name'] != None:
					...
					email=r['email'], rtoken=token_info['refresh_token'],\
					atoken=token_info['access_token'], texpiresat=token_info['expires_at'])
					u.save()
					return HttpResponseRedirect('/home')
				else:
...
def home(request):
	...
	# check the recmanded user history to see if he played it
	seen = []
	repeated = []
	seentop = []
			...
			if item.recdado.historyperm == True:
				...
				for item in recdados:
					if item in repeated:
						...
						spsp = spotipy.Spotify(item.recdado.atoken)
						history = spsp.current_user_recently_played()
						for x in range(50):
				...
				for idx, items in enumerate(toptracks2):
					seentop.append(items[idx]['uri'])
		except:
			pass
	...
	return render(request, 'spotify/home.html', {'userinfo' : userinfo,  \
...
def recommend(request):
	...
	try:
		...
		pass
	return render(request, 'spotify/recommend.html', {'sesusername' : request.session["username"],\
...
def listento(request):
	...
	if request.method == 'POST':
		...
		return render(request, 'spotify/luckyguy.html')
...
def listentothis(request, genurl):
	...
	if request.method == "POST":
		...
		try:
			deleteold = Answer.object.get(answer=rec.answer)
			deleteold.delete()
		except:
			pass
		rec.answer = u
	...
	if (rec.recdado) == (rec.recmander):
		messages.info(request, 'Dude, that request was done by you. Lets just go back')
		return HttpResponseRedirect('/home')

	# signal that the user has seen the rec
	rec.visto = True
	rec.save()
	...
	return render(request, 'spotify/gotrecd.html', {'rec':rec, 'musicinfo' : musicinfo,\
...
def addmusic(request):
	if request.method == "POST":
		...
		try:
			...
			# may be a track
			try:
				spotid = request.POST.get("songid")
				trackid=spotid.split(":")
				trackid=trackid[2]
				song = [trackid]
...
def answer(request):
	...
	# I want to embed it to play
		...
		return render(request, 'spotify/answer.html', {'embedurl' : embedurl, 'recdados' : recdados,})
